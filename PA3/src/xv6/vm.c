/* 	CS450 Fall 2019
  ...
  Added three new functions, sharedmempage(), freesharedpage() and sharepageinit().  
*/
...
sharedmempage(int key, int numPages, struct proc* proc)
{
  ...
  if(usedkeys[key]==0){
	    ...
	    for(page=0; page < numPages; page++){ //for each requested page
	      ...
	      address = (void*)P2V(memory);

	      proc->page_va_addr[key][page] = address;
	      
	      proc->top =(int) address;
	    ...
	    if(firstCall){
		    ...
		    int tnumPages;
		  	if(numPages>pagecounts[key]) tnumPages = pagecounts[key];
		      	else tnumPages = numPages;	
		    
		    int page;
		    for(page=0;page<tnumPages;page++){

        //change the address of the next avalible virtual page in the calling process' address space 

          address = (void*)P2V(pagepaddresses[key][page]);
	    ...
	    }else
	    {
		    //if this process has already requested with this key, simply return the address stored in proc->page_va_addr[][]
			 return (void*)(myproc()->page_va_addr[key][pagecounts[key]]);
		  
	    }   
	    
  }
	
  //Finally, update the reference count of the shared page if the calling process has not already used this shared page
  if(proc->keys[key]==0){
